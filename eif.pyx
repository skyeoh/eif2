# Cython wrapper for Extended Isolation Forest

# distutils: language = C++
# distutils: sources  = eif.cxx

import cython
import numpy as np
cimport numpy as np

np.import_array()

cdef extern from "eif.hxx":
    cdef cppclass iForest:
        iForest (int, int, int, int, int)
        void CheckExtensionLevel ()
        void fit (double*, int)
        void predict (double*, double*, int)

cdef class iForest:
    cdef int size_Xfit
    cdef iForest* thisptr

    def __cinit__ (self, int dim, int ntrees, int sample, int limit=None, int exlevel=0):
        if limit is None:
            limit = 0
        self.thisptr = new iForest (dim, ntrees, sample, limit, exlevel)

    def __dealloc__ (self):
        del self.thisptr

    def CheckExtensionLevel (self):
        self.thisptr.CheckExtensionLevel ()

    @cython.boundscheck(False)
    @cython.wraparound(False)
    def fit (self, np.ndarray[double, ndim=2, mode="c"] Xfit not None):
        self.size_Xfit = Xfit.shape[0]
        self.thisptr.fit (<double*> np.PyArray_DATA(Xfit), self.size_Xfit)

    @cython.boundscheck(False)
    @cython.wraparound(False)
    def predict (self, np.ndarray[double, ndim=2, mode="c"] Xpred=None):
        if Xpred is None:
            np.ndarray[double, ndim=1, mode="c"] S = np.empty(self.size_Xfit, dtype=np.float64, order='C')
            self.thisptr.predict (<double*> np.PyArray_DATA(S))
        else:
            np.ndarray[double, ndim=1, mode="c"] S = np.empty(Xpred.shape[0], dtype=np.float64, order='C')
            self.thisptr.predict (<double*> np.PyArray_DATA(S), <double*> np.PyArray_DATA(Xpred), Xpred.shape[0])
        return S
